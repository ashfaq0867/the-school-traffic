    ...
    <main class="px-6 xl:px-64">
        ...
        <section class="top py-8">
            <div class="flex item-center justify-center space-x-4 flex-wrap md:flex-nowrap px-8 xl:px-96">
                <a href="{{ route('final.quiz', request()->id)  }}"
        ...
        <section class="quiz_section bg-slate-200 rounded-xl py-8">
            ...
            <form class="mx-auto px-8" method="post" action="{{route('quiz', request()->id)}}" id="quiz_form">
                @csrf
                @php $count = 1; $count_multi = 1; @endphp
                @forelse($data->quiz as $ind => $quiz)
                    ...
                    @if($quiz->type === 'bool')
                            ...
                            @forelse($quiz->answers as $option)
                                <div class="flex items-center">
                                    ...
                                    <input id="{!! 'bool_'. $option->id !!}"
                                           ...
                                           {{-- @dd($getIds); --}}
                                           {{--										   {{ old('bool_'.$quiz->id) == $letter . '_' . $option->id ? 'checked' : '' }}--}}
                                           {{  (isset($getIds) && $getIds[$ind]["answer_id"] == $option->id) ? 'checked' : '' }}
                                           required>
                                ...
                                @php
                                    if (isset($getIds) && $quiz->id == $getIds[$ind]["quiz_id"]){
                                        if($getIds[$ind]["answer_id"] == $option->id && $getIds[$ind]["result"] == 1){
                                            $correctAnswerbool = $letter;
                                        }else if($getIds[$ind]["answer_id"] == $option->id && $getIds[$ind]["result"] == 0){
                                            $wrongAnswerbool = $letter;
                                        }
                                    }
                                @endphp

                                {{-- @if(isset($getIds))
                                     @if($quiz->id == $getIds[$ind]["quiz_id"])
                                         @if($getIds[$ind]["result"] == 1)
                                             $correctAnswer = $letter;
                                         @endif
                                         @dump($getIds[$ind]["user_id"])
                                         @dump($getIds[$ind]["part_id"])
                                         @dump($getIds[$ind]["quiz_id"])
                                         @dump($getIds[$ind]["answer_id"])
                        ...
                        @php $count++; @endphp
                    @elseif($quiz->type === 'multi')
                        @if($count_multi === 1)
                            <h2 class="text-2xl font-bold my-6">Multi Choice</h2>
                        @endif
                        <div class="mb-4">
                            <label class="block text-gray-700 font-bold mb-2 xl:text-2xl">
                                {!! $count_multi !!}. {!! $quiz->title !!}
                            </label>
                            ...
                            @forelse($quiz->answers as $option)
                                <div class="flex items-center mt-2">
                                    {!! $letter = $letters[$counter] !!} &nbsp;
                                    <input id="{!! 'multi_'. $option->id !!}"
                                           name="multi_{!! $quiz->id !!}"
                                           type="radio"
                                           class="form-radio"
                                           disabled="true"
                                           value="{!! $letter . '_' . $option->id !!}"
                                           {{--										   {{ old('multi_'.$quiz->id) == $letter . '_' . $option->id ? 'checked' : '' }}--}}
                                           {{  (isset($getIds) && $getIds[$ind]["answer_id"] == $option->id) ? 'checked' : '' }}
                                           required>
                                    <label class="form-radio-label ml-3 xl:text-xl"
                                           for="{!! 'multi_'. $option->id !!}">
                                        {!! $option->answer !!}
                                    </label>
                                </div>
                            ...
                            @error('multi_'.$quiz->id)
                            <p class="text-red-600 text-xl">Answer is required.</p>
                            @enderror

                        </div>
                        @php $count_multi++; @endphp
                    @endif
                @empty
